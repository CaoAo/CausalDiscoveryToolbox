...
def run_CGNN_decomposable_tf(df_data, type_variables, graph, list_parents, target, idx=0, run=0,  **kwargs):
    ...
    if gpu:
        ...
        model = CGNN_decomposable_tf(df_data.shape[0], data_target.shape[1], data_parents.shape[1],
                                     data_all_other_variables.shape[1], gamma, run, idx, **kwargs)
        model.train(data_target, data_all_other_variables,
                    data_parents, **kwargs)
        score_node = model.evaluate(
            data_target, data_all_other_variables, data_parents, **kwargs)
...
class CGNN_decomposable(GraphModel):
    ...
    def orient_directed_graph(self, data, dag, type_variables=None, alg='HC', **kwargs):
    ...
    def orient_undirected_graph(self, data, umg, type_variables=None,
                                ilp_mode=True, name_graph="", saved_blocks=None, **kwargs):
